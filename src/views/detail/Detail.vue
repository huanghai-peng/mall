<template>
  <div id="detail">
    <detail-nav-bar class="detail-nav-bar" @themeTopClik="themeTopClik" ref="nav"></detail-nav-bar>
    <scroll class="content" ref="scroll" @scrollClick="scrollClick" :probe-type="3">
      ...
      <detail-goods-info :detailGoods="detailGoods" @imgLoad="imgLoad"></detail-goods-info>
      <detail-goods-params :goodsParam="goodsParam" ref="params"></detail-goods-params>
...
import {getDetail,GoodsInfo,Shop,GoodsParam,recommend} from 'network/detail.js'
  ...
  methods:{
    ...
    scrollClick(opation){
      ...
      for (let i=0;i<length-1;i++){
        ...
        if(this.currentIndex !== i && (option >= this.themeTopY[i] && option < this.themeTopY[i+1])){
          this.currentIndex = i;
          this.$refs.nav.currentIndex = this.currentIndex;
        }
      }
